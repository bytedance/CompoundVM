...
public class ParsingTest {
    private static Method parseIntMethod = null;

    static {
        try {
            parseIntMethod = Formatter.class.getDeclaredMethod("parseInt", CharSequence.class,
                    int.class,
                    int.class,
                    int.class);
    ...
    private static void checkNumberFormatException(String val, int start, int end, int radix) {
        int n = 0;
        try {
            n = (Integer) parseIntMethod.invoke(null, val, start, end, radix);
            System.err.println("parseInt(" + val + ", " + start + ", " + end + ", " + radix +
                    ") incorrectly returned " + n);
            throw new RuntimeException();
        } catch (InvocationTargetException nfe) {
            if (nfe.getCause() instanceof NumberFormatException) {
                ; // Expected
            } else {
                throw new RuntimeException();
            }
    ...
    private static void checkNull(int start, int end, int radix) {
        int n = 0;
        try {
            n = (Integer) parseIntMethod.invoke(null, null, start, end, radix);
            System.err.println("parseInt(null, " + start + ", " + end + ", " + radix +
                    ") incorrectly returned " + n);
            throw new RuntimeException();
        } catch (InvocationTargetException nfe) {
            if (nfe.getCause() instanceof NullPointerException) {
                ; // Expected
            } else {
                throw new RuntimeException();
            }
