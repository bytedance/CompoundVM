...
public class StringRepeat {
    private static Method repeatMethod = null;

    static {
        try {
            repeatMethod = String.class.getMethod("repeat", int.class);
        } catch (Exception e) {
    ...
    static void test1() throws Exception {
        for (int repeat : REPEATS) {
            for (String string : STRINGS) {
                ...
                verify((String)repeatMethod.invoke(string, repeat), string, repeat);
            }
        }
    }
    ...
    static void test2() throws Exception {
        ...
        try {
            repeatMethod.invoke("abc", Integer.MAX_VALUE - 1);
            throw new RuntimeException("No exception for large repeat count");
        } catch (InvocationTargetException nfe) {
            if (nfe.getCause() instanceof OutOfMemoryError) {
                ; // Expected
            } else {
                throw new RuntimeException();
            }
