...
JVM_ENTRY(void, JVM_Sleep(JNIEnv* env, jclass threadClass, jlong millis))
  JVM_SleepNanos(env, threadClass, millis * NANOSECS_PER_MILLISEC);
JVM_END
...
JVM_LEAF(jint, JVM_Recv(jint fd, char *buf, jint nBytes, jint flags))
  //%note jvm_r6
  return (jint)os::recv(fd, buf, (size_t)nBytes, (uint)flags);
JVM_END


JVM_LEAF(jint, JVM_Send(jint fd, char *buf, jint nBytes, jint flags))
  //%note jvm_r6
  return (jint)os::send(fd, buf, (size_t)nBytes, (uint)flags);
JVM_END
...
JVM_LEAF(jint, JVM_Connect(jint fd, struct sockaddr *him, jint len))
  //%note jvm_r6
  return (jint)os::connect(fd, him, (socklen_t)len);
JVM_END
...
JVM_ENTRY(jclass, JVM_LoadClass0(JNIEnv *env, jobject receiver,
  ...
  jclass result =  find_class_from_class_loader(env, name, true, h_loader,
                                                false, thread);
  if (result != NULL) {
...
JVM_ENTRY(jobject, JVM_InitProperties(JNIEnv *env, jobject properties))
  ...
  // from setting the value with a -D option, as requested.
  {
    if (FLAG_IS_DEFAULT(MaxDirectMemorySize)) {
      ...
      char as_chars[256];
      jio_snprintf(as_chars, sizeof(as_chars), UINT64_FORMAT, MaxDirectMemorySize);
